/**
 * Minimal type definition for nanostore-like stores.
 * This avoids a direct dependency on nanostores types.
 */
 ...
 * ```
 */
export function fromStore<T>(store: Partial<StoreApi<T>>) {
	...
	$effect(() => {
		...
		const unsubscribe = store.listen((newValue: T) => {
			value = newValue;
		});
